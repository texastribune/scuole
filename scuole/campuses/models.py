...
class Campus(models.Model):
    ...
    phone_number = PhoneNumberField(_("Campus phone number"), max_length=20, null=True)
    phone_number_extension = models.CharField(
        _("Phone number extension"), max_length=4, blank=True, default=""
    )
    website = models.URLField(_("Campus website"), blank=True, default="")
    ...
    district = models.ForeignKey(
        District, on_delete=models.CASCADE, related_name="campuses"
    )
    county = models.ForeignKey(
        County, on_delete=models.CASCADE, related_name="campuses"
    )
    ...
    def get_absolute_url(self):
        ...
        return reverse(
            "districts:campus",
            kwargs={"slug": self.slug, "district_slug": self.district.slug},
        )
    ...
    def as_geojson(self):
        return to_geojson_feature(
            self, "coordinates", ["name", "tea_id"], simplify=False
        )


class CampusStats(StatsBase, ReferenceBase):
    campus = models.ForeignKey(Campus, on_delete=models.CASCADE, related_name="stats")
    year = models.ForeignKey(
        SchoolYear, on_delete=models.CASCADE, related_name="campus_stats"
    )
