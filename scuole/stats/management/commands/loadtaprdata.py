...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.school_year, _ = SchoolYear.objects.get_or_create(name=options["year"])
    ...
    def load_data(self):
        file_names = ["{}.csv".format(schema) for (schema, _) in SCHEMA.items()]
        ...
        for m in MAPPING:
            ...
            for row in self.data_list_joiner(id_match, data):
                ...
                model = self.get_model_instance(name, identifier, active_model)

                if not model:
                    continue

                payload = {"year": self.school_year, "defaults": {}}
                ...
                for schema_type, schema in SCHEMA.items():
                    if name in no_reference_file and schema_type == "reference":
                        continue
                    payload["defaults"].update(
                        self.prepare_row(m["short_code"], schema_type, schema, row)
                    )
