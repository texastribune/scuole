...
const processSass = async filepath => {
  ...
  try {
    ...
    throw {
      source: 'sass',
      type: 'error',
      file: path.relative(paths.projectDirectory, err.file),
  ...
  const relativePath = replaceExtension(
    path.relative(paths.staticSrcStyles, filepath),
    '.css'
  );

  // determine the new path relative to the destination location
  const newPath = path.join(paths.staticDistStyles, relativePath);
